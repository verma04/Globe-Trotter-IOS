  ...
  if (error) return  <Text>Something Bad Happened</Text>
    ...
    return (
      <View style={{flex:1, backgroundColor:colors.borderColor}} >
         <ActionBarImage2 navigation={navigation} title={"Bespoke Trips"} />
                 <ScrollView>
     
            <View style={styles.containerStyle}>
            <View style={styles.center}>
              {fin.getConciergeItems.map(data =>

<TouchableOpacity key={data.id} onPress={()=> navigation.navigate('ViewItem', { name: data.title , data:data  , all:fin.getConciergeItems   }) }  style={[styles.box ,{
