...
const VillaList = ({route, navigation}) => {
  const { colors } = useTheme();
  const HOMEPAGE_QUERY = `  query GetItemByCategory($id: String) {
    getItemByCategory(id: $id) {
      id
      category
      subCategory
      img {
      ...
      <View style={{flex:1, backgroundColor:colors.borderColor}} >
        <ActionBarImage2 navigation={navigation} title={route.params.name} />
            ...
            <View style={styles.center}>
              {fin.getItemByCategory.sort((a, b) => a.title.localeCompare(b.title)).map(data =>

<TouchableOpacity key={data.id} onPress={()=> navigation.navigate('ViewItem', { name: data.title , data:data  , all:fin.getItemByCategory }) }  style={[styles.box ,{
